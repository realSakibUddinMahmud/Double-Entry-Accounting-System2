...
{
    public static $eventName = 'EXPENSE';
    public function index()
    {
        ...
        if (request()->filled('download') && request('download') === 'YES') {
            ...
            $pdfFileName = 'Expenses-Report-' . $start_date . '-' . $end_date . '-' . uniqid().'-'.env('APP_NAME');
            $pdf = PDF::setPaper('a4', 'portrait')
                ->loadView('de-accounting::expenses.download-pdf', [
                    'journals' => $journals,
        ...
        return view('de-accounting::expenses.index', compact('journals'));
    }

    public function create()
    {
        return view('de-accounting::expenses.create');
    }
    ...
    public function delete($id)
    {
        ...
        return redirect()->back()->with('error', 'Failed to delete the expense.');
    }
