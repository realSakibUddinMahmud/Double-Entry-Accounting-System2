...
{
    public static $eventName = 'LOAN-INVESTMENT';
    public function index()
    {
        ...
        if (request()->filled('download') && request('download') === 'YES') {
            ...
            $pdfFileName = 'Loan / Investment-Report-' . $start_date . '-' . $end_date . '-' . uniqid().'-'.env('APP_NAME');
            $pdf = PDF::setPaper('a4', 'portrait')
                ->loadView('de-accounting::loan-investments.download-pdf', [
                    'journals' => $journals,
        ...
        return view('de-accounting::loan-investments.index', compact('journals'));
    }
    ...
    {
        return view('de-accounting::loan-investments.create');
    }
    ...
    public function delete($id)
    {
        ...
        if ($journal) {
            ...
            if ($result['status'] && $result['status'] == true) {
                return redirect()->back()->with('success', 'Loan / investment deleted successfully.');
            } 
        }

        return redirect()->back()->with('error', 'Failed to delete loan investment.');
    }
