...
class LoanInvReturnController extends Controller
{
    public static $eventName = 'LOAN-INVRETURN';
    public function index()
    {
        ...
        if (request()->filled('download') && request('download') === 'YES') {
            ...
            $pdfFileName = 'Loan / Inv. Return-Report-' . $start_date . '-' . $end_date . '-' . uniqid().'-'.env('APP_NAME');
            $pdf = PDF::setPaper('a4', 'portrait')
                ->loadView('de-accounting::loan-invreturns.download-pdf', [
                    'journals' => $journals,
            ...
            // return view('de-accounting::fund-transfers.download-pdf', compact('journals'));
        }
            
        $journals = $query->latest()->paginate()->appends(request()->query());
        

        return view('de-accounting::loan-invreturns.index', compact('journals'));
    }
    ...
    {
        return view('de-accounting::loan-invreturns.create');
    }
    ...
    public function delete($id)
    {
        ...
        if ($journal) {
            ...
            if ($result['status'] && $result['status'] == true) {
                return redirect()->back()->with('success', 'Loan / inv. return deleted successfully.');
            } 
        }

        return redirect()->back()->with('error', 'Failed to delete loan / inv. return.');
    }
