...
class FundTransferComponent extends Component
{
    ...
    public $sourceAccountRootType = 1;
    public $destinationAccountRootType = 1;
    public $eventName = 'FUND-TRANSFER';
    ...
    protected $rules = [
        ...
        'destination_accountable_type' => 'required',
        'destination_accountable_id' => 'required',
        'date' => 'required|date',
    ...
    public function changeDestinationAccountableType($value)
    {
        $accountables = DeAccount::$accountables;
        ...
        if ($match && class_exists($match['class'])) {
            ...
            // Order by title if exists, otherwise name
            if (in_array('title', $columns)) {
                $this->destination_accountable_data = $className::orderBy('title')->get();
            } elseif (in_array('name', $columns)) {
                $this->destination_accountable_data = $className::orderBy('name')->get();
            } else {
                $this->destination_accountable_data = $className::all();
            }
        } else {
            $this->destination_accountable_data = [];
        }
    }
    ...
    public function changeDestinationAccountableId($value)
    {
    ...
    public function render()
    {
        ...
        return view('de-accounting::livewire.fund-transfer.create');
    }
