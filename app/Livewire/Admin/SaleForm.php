...
class SaleForm extends Component
{
    ...
    public $product_id, $unit_id, $quantity, $per_unit_price, $per_unit_cogs, $payment_account_id;
    public $product_items = [];
    public $total_amount = 0, $paid_amount = 0, $due_amount = 0, $payment_status = 'Pending',  $total_tax = 0;
    public $note = '';
        ...
        if ($this->mode === 'edit' && $this->saleId) {
            ...
            foreach ($sale->items as $item) {
                $this->product_items[] = [
                    ...
                    'per_unit_cogs' => $item->per_unit_cogs,
                    'tax_id' => $item->tax_id ?? optional($item->product)->tax_id,
                    'total' => $item->total,
                ];
            }
    ...
    public function addItem()
    {
        ...
        if ($this->store_id && $product) {
            $productStore = $product->productStores()
                ...
                ->first();
            if ($productStore) {
                $taxId = $productStore->tax_id ?? null;
                $taxMethod = $productStore->tax_method ?? 'exclusive';
            }
        }
        ...
        $item = [
            ...
            'quantity' => $this->quantity,
            'per_unit_price' => $this->per_unit_price,
            'per_unit_cogs' => $this->per_unit_cogs,
    ...
    public function calculateTotals()
    {
        $subtotal = collect($this->product_items)->sum(function($item) {
            ...
            $price = (float)($item['per_unit_price'] ?? 0);
            $taxAmount = $this->calculateItemTaxAmount($item);

            if (($item['tax_method'] ?? 'exclusive') === 'exclusive') {
                return ($qty * $price) + $taxAmount;
            } else {
                return $qty * $price; // already includes tax
            }
        });

        $this->total_tax = collect($this->product_items)->sum(function($item) {
            return $this->calculateItemTaxAmount($item);
        });
        ...
        try {
            if ($this->mode === 'edit' && $this->saleId) {
                ...
                if ($task) {
                    ...
                    if ($salePaymentJournalCount > 1) {
                        ...
                        foreach ($task->journals()->where('transaction_type', 'SALE-TAX')->get() as $journal) {
                            if ($journal) {
                                $result = DE::delete($journal);
                                if ($result['status'] && $result['status'] != true) {
                                    DB::rollBack();
            ...
            foreach ($this->product_items as $item) {
                SaleItem::create([
                    ...
                    'quantity' => $item['quantity'],
                    'per_unit_price' => $item['per_unit_price'],
                    'per_unit_cogs' => $item['per_unit_cogs'] ?? 0, // Add per_unit_cogs
                    'total' => $item['total'],
                    'tax_id' => $item['tax_id'] ?? null,
                    'tax_amount' => $this->calculateItemTaxAmount($item),
                ]);
            }
            ...
            if ($task) {
                ...
                if($salePaymentJournalCount < 1) {
                    ...
                    if ($this->payment_status === 'Paid') {
                        // Initialize variable for paid payment status
                        $amount = $this->paid_amount - $this->total_tax;
                        if ($this->handlePaymentAccounting($sale,$amount, $task->id) === false) {
                            return;
                        }
                    }
                    elseif ($this->payment_status === 'Partial') {                        
                        
                        if($this->paid_amount >= $this->due_amount){
                            $paid_amount = $this->paid_amount - $this->total_tax;
                            $due_amount = $this->due_amount;
                        }else{
                            $due_amount = $this->due_amount - $this->total_tax;
                            $paid_amount = $this->paid_amount;
                        }
                        
                        if ($this->handlePaymentAccounting($sale, $paid_amount, $task->id) === false) {
                            return;
                        }
                        if ($this->handleDueAccounting($sale, $due_amount, $task->id) === false) {
                            return;
                        }
                    }
                    else{
                        // Initialize variable for pending payment status
                        $due_amount = $this->due_amount - $this->total_tax;
                        if ($this->handleDueAccounting($sale,$due_amount, $task->id) === false) {
                            return;
                        }
    ...
    public function selectProduct($productId)
    {
        ...
        // Get tax_id and tax_method from product_store
        $taxId = null;
        $taxMethod = 'exclusive';
        ...
        if ($this->store_id && $product) {
            ...
            $cogsAvg = optional(CogsAvgView::where('product_id', $product->id)
                ->where('store_id', $this->store_id)
                ->select('cogs_avg')
                ->first())->cogs_avg ?? 0;
        }
        ...
        $this->product_items[] = [
            ...
            'per_unit_cogs' => $cogsAvg, // Assuming cogs_avg is available in product
            'total' => $salePrice,
            'tax_id' => $taxId,            // <-- Add this line
            'tax_method' => $taxMethod,    // <-- Add this line
        ];
    ...
    public function handleCOGSAccounting($sale,$taskId)
    {
        ...
        $totalCOGS = collect($sale->items)->sum(function ($item) use ($sale) {
            return $item->quantity * $item->per_unit_cogs;
        });
    ...
    public function handleTaxAccounting($sale, $taskId, $receivableAc)
    {
        $store = $sale->store;
        $company_id = Auth::user()->tenant_id;
        $company = Company::findOrFail($company_id);

        $salesTaxAccount = $company->accounts()
            ->where('title', 'Sales Tax Payable')
            ->where('root_type', 3)
            ->where('account_type_id', 1)
            ->whereNotNull('parent_id')
            ->first();
        ...
        if (!$salesTaxAccount || !$receivableAc) {
            DB::rollBack();
            session()->flash('error', 'Sales Tax Payable or Receivable account not found for this store.');
            return false; // Stop further execution
        }

        $sourceAccount = $salesTaxAccount;
        $destinationAccount = $receivableAc;
        ...
        $requestedData['destination_transactionable_type'] = $destinationAccount->accountable ? get_class($destinationAccount->accountable) : null;
        $requestedData['note'] = $sale->note ?? null;
        $requestedData['amount'] = $sale->total_tax ?? 0;
        $requestedData['journalable_type'] = Sale::class;
        $requestedData['journalable_id'] = $sale->id;
    ...
    public function handlePaymentAccounting($sale, $amount, $taskId)
    {
        $store = $sale->store;
        ...
        if($sale->payment_status === 'Paid'){
            if($sale->total_tax > 0){
                if($this->handleTaxAccounting($sale, $taskId, $assetAccount) === false){
                    return;
                }
            }
        }elseif($sale->payment_status === 'Partial'){
            if($amount >= $sale->total_tax){
                if($this->handleTaxAccounting($sale, $taskId, $assetAccount) === false){
                    return;
                }
            }
    ...
    public function handleDueAccounting($sale, $amount, $taskId)
    {
        $store = $sale->store;
        ...
        if($sale->payment_status === 'Pending'){
            if($sale->total_tax > 0){
                if($this->handleTaxAccounting($sale, $taskId, $customerReceivableAccount) === false){
                    return;
                }
            }
        }elseif($sale->payment_status === 'Partial'){
    ...
    public function calculateItemTaxAmount($item)
    {
        $qty = (float)($item['quantity'] ?? 0);
        $price = (float)($item['per_unit_price'] ?? 0);
        $taxAmount = 0;
        ...
        if (!empty($item['tax_id'])) {
            $tax = $this->taxes->where('id', $item['tax_id'])->first();
            if ($tax && isset($tax->rate)) {
                $taxRate = (float)$tax->rate;
                if (($item['tax_method'] ?? 'exclusive') === 'exclusive') {
                    $taxAmount = ($price * $qty) * ($taxRate / 100);
                } else { // inclusive
                    $taxAmount = ($price * $qty) - (($price * $qty) / (1 + ($taxRate / 100)));
                }
            }
        }
        return $taxAmount;
