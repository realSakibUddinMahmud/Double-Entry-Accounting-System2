...
class PurchaseForm extends Component
{
    ...
    public $product_id, $unit_id, $quantity, $per_unit_cost , $per_unit_cogs;
    ...
    public function mount($purchaseId = null, $mode = 'create')
    {
        ...
        if ($this->mode === 'edit' && $this->purchaseId) {
            ...
            $this->total_tax = $purchase->total_tax;
            ...
            foreach ($purchase->items as $item) {
                ...
                $this->product_items[] = [
                    ...
                    'per_unit_cogs' => $item->per_unit_cogs, // <-- add this line
                    'tax_id' => $item->tax_id ?? optional($item->product)->tax_id, // <-- add this line
                    'total' => $item->total,
                ];
            }
        }
    ...
    public function addItem()
    {
        ...
        $item = [
            ...
            'base_unit_id' => $product ? ($product->productStores()->where('store_id', $this->store_id)->first()->base_unit_id ?? $this->unit_id) : $this->unit_id,
            'quantity' => $this->quantity,
            'per_unit_cost' => $this->per_unit_cost,
            'per_unit_cogs' => $this->per_unit_cogs, // Use product COGS if available
            'tax_id' => $product ? $product->tax_id : null,
            'tax_method' => $product ? $product->tax_method : 'exclusive', // default to exclusive
    ...
    public function calculateTotals()
    {
        ...
        // Calculate total tax from all product items
    ...
    public function save()
    {
        ...
        $validator = Validator::make([
            ...
            'product_items.*.per_unit_cogs' => ['required', 'numeric', 'min:0.01', 'regex:/^\d+(\.\d{1,2})?$/'],
        ]);
        ...
        try {
            if ($this->mode === 'edit' && $this->purchaseId) {
                ...
                if ($task) {
                    ...
                    foreach ($task->journals()->where('transaction_type', 'PURCHASE-TAX')->get() as $journal) {
                        if ($journal) {
                            $result = DE::delete($journal);
                            if ($result['status'] && $result['status'] != true) {
                                DB::rollBack();
            ...
            foreach ($this->product_items as $item) {
                ...
                if (!empty($item['tax_id'])) {
                    ...
                    if ($tax && isset($tax->rate)) {
                        ...
                        $taxAmount = ($item['per_unit_cost'] * $item['quantity']) * ($tax->rate / 100);
                    }
                }
            ...
            if ($this->total_tax > 0) {
                $this->purchaseTaxAccounting($purchase->id, $task->id);
            }
    ...
    public function purchaseTaxAccounting($purchaseId, $taskId)
    {
        $purchase = Purchase::findOrFail($purchaseId);
        ...
        $purchaseTaxAccount = $company->accounts()
            ->where('title', 'Purchase Tax')
            ->where('root_type', 1)
            ->where('account_type_id', 1)
            ->whereNotNull('parent_id')
            ->first();
        ...
        if (!$purchaseTaxAccount || !$payableAccount) {
            DB::rollBack();
            session()->flash('error', 'Tax or Payable account not found for this supplier.');
            return false; // Stop further execution
        }

        $sourceAccount = $purchaseTaxAccount;
        $destinationAccount = $payableAccount;
        ...
        $requestedData['destination_transactionable_type'] = $destinationAccount->accountable ? get_class($destinationAccount->accountable) : null;
        $requestedData['note'] = $purchase->note ?? null;
        $requestedData['amount'] = $purchase->total_tax ?? 0;
        $requestedData['journalable_type'] = Purchase::class;
        $requestedData['journalable_id'] = $purchase->id;

        $deType = 'UPUP';
        $eventName = 'PURCHASE-TAX';
        DE::store($sourceAccount, $destinationAccount, $requestedData, $deType, $taskId, $eventName);
    ...
    public function selectProduct($productId)
    {
        ...
        if ($this->store_id && $product) {
            ...
            if ($productStore) {
                ...
                $productCogs = $productStore->cogs ?? 0;
                $purchaseUnitId = $productStore->purchase_unit_id ?? $unitId;
                $baseUnitId = $productStore->base_unit_id ?? $unitId;
                $taxId = $productStore->tax_id ?? $product->tax_id;
                $taxMethod = $productStore->tax_method ?? $product->tax_method ?? 'exclusive';
            }
        }
        ...
        $this->product_items[] = [
            ...
            'per_unit_cogs' => $productCogs, // Use product COGS if available
    ...
    public function calculateItemTotal($item)
    {
        ...
        $cost = (float)($item['per_unit_cost'] ?? 0);
        $taxAmount = 0;
        ...
        if (!empty($item['tax_id'])) {
            ...
            if ($tax && isset($tax->rate)) {
                ...
                if (($item['tax_method'] ?? 'exclusive') === 'exclusive') {
                    $taxAmount = ($cost * $qty) * ($taxRate / 100);
                    return ($cost * $qty) + $taxAmount;
                } else { // inclusive
                    $taxAmount = ($cost * $qty) - (($cost * $qty) / (1 + ($taxRate / 100)));
                    return $cost * $qty; // total already includes tax
                }
            }
        }
        return $cost * $qty;
