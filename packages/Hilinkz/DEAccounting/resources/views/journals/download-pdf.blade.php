...
<head>
    ...
    <title>Journal-Export To PDF</title>
...
<body>
    ...
    <body>
        @include('de-accounting::journals.pdf-data-table')
            ...
            if (isset($pdf)) {
                $pdf->page_script('
                    if ($PAGE_NUM > 1) {
                        ...
                        $pageText = $compName . ",Journals (@if(date('d-M-Y',strtotime($start_date ?? today()))!= date('d-M-Y', strtotime($end_date ?? today()))){{date('d/m/Y', strtotime($start_date))}} to {{date('d/m/Y',strtotime($end_date))}}@else{{date('d/m/Y', strtotime($start_date))}}@endif)";
                        $x = 40;
                        $y = 22;
                        $pdf->text($x, $y, $pageText, $font, $size);
                    }
