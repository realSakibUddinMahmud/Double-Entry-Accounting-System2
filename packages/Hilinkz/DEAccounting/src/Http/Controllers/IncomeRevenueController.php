...
class IncomeRevenueController extends Controller
{
    public static $eventName = 'INCOME-REVENUE';
    public function index()
    {
        ...
        if (request()->filled('download') && request('download') === 'YES') {
            ...
            $pdfFileName = 'Income/Revenue-Report-' . $start_date . '-' . $end_date . '-' . uniqid().'-'.env('APP_NAME');
            $pdf = PDF::setPaper('a4', 'portrait')
                ->loadView('de-accounting::income-revenues.download-pdf', [
                    'journals' => $journals,
        ...
        return view('de-accounting::income-revenues.index', compact('journals'));
    }
    ...
    {
        return view('de-accounting::income-revenues.create');
    }
    ...
    public function delete($id)
    {
        ...
        if ($journal) {
            ...
            if ($result['status'] && $result['status'] == true) {
                return redirect()->back()->with('success', 'Income/Revenue deleted successfully.');
            } 
        }

        return redirect()->back()->with('error', 'Failed to delete income/revenue.');
    }
